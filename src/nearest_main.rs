/// A Modified Version of the nearest node algorithm(including nodes on reference paths)
...
pub fn nearest_main(matches: &ArgMatches) -> Result<(), Box<dyn std::error::Error>> {
    info!("Running 'gfa2bin-aligner nearest'");

    // Graph file
    let graph_file = matches.get_one::<String>("gfa").unwrap().as_str();

    // Output file
    let output_file: String = if let Some(out) = matches.get_one::<String>("output") {
        out.clone()
    } else {
        let graph_path = Path::new(graph_file);
        let parent = graph_path.parent().unwrap_or_else(|| Path::new("."));
        let default_pathbuf = parent.join("alignment.tsv");
        default_pathbuf.to_string_lossy().into_owned()
    };

    let threads = matches.get_one::<String>("threads").unwrap().as_str();
    ...
    info!(
        ...
        if matches.contains_id("nodes") {
            matches.get_one::<String>("nodes").unwrap().as_str()
        } else {
            "All nodes"
        }
    );
    info!(
        ...
        if matches.contains_id("references") {
            matches.get_one::<String>("references").unwrap().as_str()
        } else {
            "None"
        }
    );
    ...
    if matches.contains_id("nodes") {
        ...
        requested_nodes = read_input(matches.get_one::<String>("nodes").unwrap().as_str()).unwrap();
    } else {
        info!("No nodes provided, all nodes will be considered");
        requested_nodes = graph.segments.iter().map(|x| x.id).collect();
    }
    info!("Number of requested nodes: {}", requested_nodes.len());
    ...
    info!("Number of reference paths: {}", ref_list.len());

    let keep_ref_nodes = matches.contains_id("keep-ref");
    ...
    info!("Nearest node computation completed successfully");
    Ok(())
}
